...
**On mobile / touch devices** the game auto-switches to a minimal on-screen control set (rotate to
landscape): a red **GAS** button on the right, **JUMP** on the left, and a **⟳** restart in the centre.
It also drops to a lighter render tier (reduced resolution, no SSAO, smaller shadows) to hold framerate.
Append `?touch=1` to the URL to force the touch layer on any device.
